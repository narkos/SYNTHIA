...
public class PlayerControllerBoxed : MonoBehaviour
{
    ...
    // Parent
    private PlayAreaController _parentController;
    ...
    void Start()
    {
        ...
        _parentController = GetComponentInParent<PlayAreaController>();
    }
    ...
    void Update()
    {
        ...
        if((Input.GetAxisRaw("Horizontal") != 0.0f || Input.GetAxisRaw("Vertical") != 0.0f) && Time.time - _startTime > 0.25f)
        {
            ...
            _wantedPosition = transform.localPosition + GetVector3(Input.GetAxisRaw("Horizontal"), Input.GetAxisRaw("Vertical")) * _range;
        ...
        if(_isMoving)
        {
            ...
            if (_journey <= jumpDuration + 0.01f)
            {
                ...
                transform.localPosition = _wantedPosition;
            }
            else {
                _isMoving = false;
                _isReturning = true;
        ...
        if(_isReturning)
        {
            ...
            if(_journey <= returnDuration)
            {
                ...
                transform.localPosition = Vector3.LerpUnclamped(_wantedPosition, _origin, percent);
            } else {
